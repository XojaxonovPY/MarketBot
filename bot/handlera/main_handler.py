...
async def main_handler(message:Message):
    ...
    text = [InlineKeyboardButton(text=category.name, callback_data=f"category_{category.id}") for category in categories]
...
async def com_handler(message:Message):
    await message.answer(text='99-277-12-81')
...
class CustomMiddleware(BaseMiddleware):
    async def __call__(self, handler, event: Message, data: dict):
        ...
        if not_join_channels:
            buttons = [InlineKeyboardButton(text=f"channel {i}" , url=channel.link) for i, channel in enumerate(not_join_channels,1)]
